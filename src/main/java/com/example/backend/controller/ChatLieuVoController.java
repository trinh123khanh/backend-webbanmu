...
@RestController
@RequestMapping("/chat-lieu-vo")
@CrossOrigin(origins = {"http://localhost:4200", "http://localhost:3000"})
public class ChatLieuVoController {

    private final ChatLieuVoService service;

    public ChatLieuVoController(ChatLieuVoService service) {
        this.service = service;
    }

    @PostMapping
    public ResponseEntity<ChatLieuVoResponse> create(@Valid @RequestBody ChatLieuVoRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(request));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ChatLieuVoResponse> update(@PathVariable Long id, @Valid @RequestBody ChatLieuVoRequest request) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ChatLieuVoResponse> getById(@PathVariable Long id) {
        return ResponseEntity.ok(service.getById(id));
    }

    @GetMapping
    public ResponseEntity<Page<ChatLieuVoResponse>> search(
